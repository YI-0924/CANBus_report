...
#define DEFAULT_TIMESTAMP_BYTE 5 // bytes 5,6,7,8
...
#define PACKGESTRFORMAT "%8X,%8X,%1d,%2X,%2X,%2x,%2x,%2X,%2X,%2x,%2x\n"
#define PACKGESTRSIZE sizeof(PACKGESTRFORMAT)
#define BUFFERSIZE PACKGESTRSIZE*1000
...
long current_speed_board = 0;
int door_status[4];
int turn_status[2];
char *model = NULL;
char data_file[256];
int frame_statistics[2048] = {};

u_int32_t bufferOffset = 0;
struct timeval now_timestamp;
long time_interval[2048] = {};	//index:ID array:time_interval
...
void update_frame_statistics() {
    system("clear");
    printf("frameID collected :\n");
    int total = 0;
    int count = 0;
    for(int i=0;i<2048;i++){
      if (frame_statistics[i] != 0){
        total += frame_statistics[i];
        printf("%3X#: %5d\t",i,frame_statistics[i]);
        if(count % 5 == 4)
          printf("\n");
        count++;
      }
    }
    double elapsed_time = ((double)clock())/CLOCKS_PER_SEC* 25.57;
    printf("\n");
    printf("time elapsed: %.2f\n", elapsed_time);
    printf("Total frame# received: %d\n", total);
...
void update_board_status(struct canfd_frame *cf, int maxdlen) {
  ...
  if (current_speed - current_speed_board > 10 || current_speed_board - current_speed > 10) {
    printf("MIGHT BE SPOOFING!!!!!\n");
    } else { update_speed(); }
 SDL_RenderPresent(renderer);
}
...
int main(int argc, char *argv[]) {
  ...
  int nbytes, maxdlen;
  int seed = 0;
  canid_t door_id, signal_id, speed_id, showstatistics_id, board_id;
  SDL_Event event;
  packgesCollected = fopen("data/packgesCollected.csv","w");
  fprintf(packgesCollected, "TS, ID, LEN, DLC0, DLC1, DLC2, DLC3, DLC4, DLC5, DLC6, DLC7\n");
...
struct timeval req_time, resp_time;	// For counting time interval
      nbytes = recvmsg(can, &msg, 0);
      gettimeofday(&req_time, NULL);	// request time
      FILE *file = fopen("banned_id.txt", "r");
	int banned_id;
	while(fscanf(file, "%x", &banned_id) != EOF){
		if(frame.can_id == banned_id){
			fclose(file);
			perror("Detect DoS Attact");
			return 1;
		}
	}
	fclose(file);
...
//      if(debug) fprint_canframe(stdout, &frame, "\n", 0, maxdlen);
           frame_statistics[frame.can_id] += 1;
      
      uint32_t tv = 0;
      for(int i=0;i<32;i+=8)
        tv |= frame.data[DEFAULT_TIMESTAMP_BYTE + i / 8] << i;
      gettimeofday(&now_timestamp, NULL);
      if(!(now_timestamp.tv_sec == 0 || (tv >= now_timestamp.tv_sec-2 && tv < now_timestamp.tv_sec + 2))){
        printf("invalid time stamp: %x, now timestamp: %x\n",tv, now_timestamp.tv_sec);
        continue;
      }
      //printf("%d\n", bufferOffset);
      ...
      // user defined id: send this id to update statistics data
      gettimeofday(&resp_time, NULL);	// response time
	time_interval[frame.can_id] = (resp_time.tv_sec - req_time.tv_sec) * 1000000L + (resp_time.tv_usec - req_time.tv_usec);

      if (frame.can_id == showstatistics_id) update_frame_statistics();
