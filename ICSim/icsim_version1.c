...
#include <sys/time.h>	//for time_interval
...
// print PID and number of packages and time
void update_frame_statistics(){
	system("clear");
	printf("frameID collected[PID#:	count:	time_interval(microseconds)] :\n");
	int total = 0;
	int count = 0;
	for(int i=0;i<2048;i++){
		if(frame_statistics[i] != 0){
			total +=frame_statistics[i];
			printf("%3X#: %5d: %5ld\n",i,frame_statistics[i],time_interval[i]);
			/*if(count %5 == 4){
				printf("\n");
			}*/
			count++;
...
int main(int argc, char *argv[]) {
  ...
  int nbytes, maxdlen;
  int seed = 0;
//  canid_t door_id, signal_id, speed_id, showstatistics_id;
